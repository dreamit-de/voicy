...
- **Normal-Modus** (`⌥` rechts halten): Aufnahme → Whisper → Auto-Insert.
- **Rewrite-Modus** (`⌥` rechts + `⌃` halten): Aufnahme → Whisper → lokales LLM (Ollama) → Auto-Insert. Der aktive Stil ist im Menü umschaltbar:
...
- **Rewrite ist optional** — ohne gewähltes Modell (oder bewusst deaktiviert) fügt `⌥⌃` den unveränderten Transkript-Text ein.
...
1. DMG öffnen, `Voicy.app` in den Ordner `Programme` ziehen.
2. **Quarantäne entfernen** (siehe Hinweis unten) — sonst startet die App nicht.
3. Voicy starten. Das Setup führt durch Sprachwahl, Berechtigungen und Modell-Download.

> [!IMPORTANT]
> **xattr-Hinweis 1 — heruntergeladene App entsperren.** Die internen Builds sind signiert, aber **nicht von Apple notarisiert**. macOS markiert heruntergeladene Apps deshalb mit dem Quarantäne-Flag und blockiert den Start („… kann nicht geöffnet werden, da Apple sie nicht auf Schadsoftware prüfen konnte"). Entferne das Flag nach dem Kopieren nach `/Programme` einmalig:
>
> ```bash
> xattr -dr com.apple.quarantine /Applications/Voicy.app
> ```
>
> Danach startet Voicy normal; die Berechtigungen (Mikrofon, Bedienungshilfen, Eingabeüberwachung) bleiben über künftige Sparkle-Updates erhalten.
...
Whisper- und Rewrite-Modelle lädt Voicy selbst herunter und legt sie unter `~/Library/Application Support/Voicy/Models/` (Whisper) bzw. im Ollama-Cache ab. Nichts davon muss manuell installiert werden.
...
In Xcode: Target `Voicy` wählen → Run (⌘R). Beim ersten Start führt das Setup durch:

1. **Sprache** — Standard ist Deutsch; danach empfiehlt Voicy die passenden Modelle.
...
> [!IMPORTANT]
> **xattr-Hinweis 2 — lokal gebaute App ohne Zertifikat starten.** Ein lokaler Build ohne Developer-ID-Zertifikat ist nur ad-hoc signiert. Solange du ihn aus Xcode startest (⌘R), ist das unkritisch. Kopierst du das gebaute `Voicy.app` aber woandershin oder gibst es weiter, greift dieselbe Gatekeeper-Quarantäne wie bei den internen Builds — dann ebenfalls einmalig entsperren:
>
> ```bash
> xattr -dr com.apple.quarantine /pfad/zu/Voicy.app
> ```
...
Audio (PCM)
  └─► Whisper (Transkription, Neural Engine)      → Text
        └─► Ollama-LLM (Rewrite, nur im ⌥⌃-Modus)  → umformulierter Text
